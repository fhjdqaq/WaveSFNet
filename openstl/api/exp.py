...
class BaseExperiment(object):
    ...
    """def test(self):
        ...
        best_path = osp.join(ckpt_dir, 'best.ckpt')

        if self.args.test:
            print_log(f"[DEBUG] test(): loading BEST checkpoint: {best_path}")
            ckpt = torch.load(best_path)
            self.method.load_state_dict(ckpt['state_dict'])
        else:
            print_log("[DEBUG] test(): NOT loading best.ckpt, "
                  "using in-memory weights (i.e. the last epoch model). "
                  f"If you want BEST, add --test to command, best path = {best_path}")
    ...
    def display_method_info(self, args):
        ...
        if args.method in ['simvp', 'tau', 'mmvp', 'wast','wavesf']:

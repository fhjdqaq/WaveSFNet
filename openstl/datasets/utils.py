...
def create_loader(dataset,
    ...
    if not is_iterable and num_aug_repeats != 0 and not _dist_ready():
        raise AssertionError("RepeatAugment is only supported when torch.distributed is initialized")

    if (distributed or _dist_ready()) and not is_iterable:
        if _dist_ready():
            if is_training:
                if num_aug_repeats:
